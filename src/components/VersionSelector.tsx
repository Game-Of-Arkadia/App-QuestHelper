...
export const VersionSelector = () => {
  ...
  const [editVersions, setEditVersions] = useState<{
    [key: string]: { name: string; color: string; folderPath: string };
  }>({});
      ...
      >
        {versionEntries.map(([id, versionData]) => (
          <button
            ...
            style={
              data.currentVersion === id
                ? { backgroundColor: versionData.color, color: 'white' }
                : {}
            }
          >
            {versionData.name}
        ...
        >
          ...
          >
            <TabsList
              ...
              style={{ gridTemplateColumns: `repeat(${versionEntries.length}, 1fr)` }}
            >
              {versionEntries.map(([id, versionData]) => (
                <TabsTrigger
                  key={id}
            ...
            {versionEntries.map(([id, versionData]) => (
              ...
              >
                ...
                >
                  ...
                  >
                    <Label
                      htmlFor={`edit-version-name-${id}`}
                    >
                      City Name
                    </Label>
                    <Input
                      id={`edit-version-name-${id}`}
                      value={editVersions[id]?.name || versionData.name}
                      onChange={(e) =>
                      ...
                      {VERSION_COLORS.map((color) => (
                        <button
                          ...
                          className={`w-10 h-10 rounded-md transition-all ${
                            (editVersions[id]?.color || versionData.color) === color
                ...
                >
                  ...
                  >
                    ...
                    <Input
                      ...
                      value={
                        editVersions[id]?.folderPath ||
                        versionData.folderPath ||
                        '~/Dialogue/<questname>/'
                      }
                      onChange={(e) =>
                        setEditVersions((prev) => ({
